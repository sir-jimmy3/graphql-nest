"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.BooksResolver = void 0;
const graphql_1 = require("@nestjs/graphql");
const books_service_1 = require("./books.service");
const book_entity_1 = require("./entities/book.entity");
const create_book_input_1 = require("./dto/create-book.input");
const remove_book_input_1 = require("./dto/remove-book.input");
const common_1 = require("@nestjs/common");
const jwt_auth_guard_1 = require("../auth/jwt-auth.guard");
const use_filter_decorator_1 = require("../decorators/use-filter.decorator");
const find_book_input_1 = require("./dto/find-book.input");
let BooksResolver = class BooksResolver {
    constructor(booksService) {
        this.booksService = booksService;
    }
    createBook(createBookInput) {
        return this.booksService.create(createBookInput);
    }
    findAll(input, findBookInput) {
        return this.booksService.findAll(findBookInput);
    }
    removeBook(removeBookInput) {
        return this.booksService.remove(removeBookInput);
    }
};
__decorate([
    (0, graphql_1.Mutation)(() => book_entity_1.Book),
    (0, common_1.UseGuards)(jwt_auth_guard_1.JwtAuthGuard),
    __param(0, (0, graphql_1.Args)('createBookInput')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [create_book_input_1.CreateBookInput]),
    __metadata("design:returntype", void 0)
], BooksResolver.prototype, "createBook", null);
__decorate([
    (0, graphql_1.Query)(() => [book_entity_1.Book], { name: 'books' }),
    (0, common_1.UseGuards)(jwt_auth_guard_1.JwtAuthGuard),
    __param(0, (0, graphql_1.Args)('findBookInput', { nullable: true })),
    __param(1, (0, use_filter_decorator_1.UseFilter)('findBookInput')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [find_book_input_1.FindBookInput,
        find_book_input_1.FindBookInput]),
    __metadata("design:returntype", void 0)
], BooksResolver.prototype, "findAll", null);
__decorate([
    (0, graphql_1.Mutation)(() => book_entity_1.Book),
    (0, common_1.UseGuards)(jwt_auth_guard_1.JwtAuthGuard),
    __param(0, (0, graphql_1.Args)('removeBookInput')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [remove_book_input_1.RemoveBookInput]),
    __metadata("design:returntype", void 0)
], BooksResolver.prototype, "removeBook", null);
BooksResolver = __decorate([
    (0, graphql_1.Resolver)(() => book_entity_1.Book),
    __metadata("design:paramtypes", [books_service_1.BooksService])
], BooksResolver);
exports.BooksResolver = BooksResolver;
//# sourceMappingURL=books.resolver.js.map